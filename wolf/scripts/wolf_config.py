"""WOLF v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

WOLF — Event-Driven / Regime-Rotation Hedge Fund. ONE producer script driven by
the WOLF_LEG env var into one of two books, each bound to its own wallet +
runtime + DSL. Both books read the SAME cross-asset REGIME detector (the shared
brain) and a book only fires when the regime AGREES with its mandate — so
capital rotates to whichever book the prevailing macro regime favors:

  WOLF_LEG=risk_on  -> Risk-On book. Fires ONLY when the cross-asset regime
                      confirms RISK-ON (equities up + oil down + gold soft +
                      BTC up + dollar soft agree). Goes LONG beaten-down beta —
                      crypto majors / high-beta alts + growth indices — riding
                      the relief-rally rotation. Wide let-it-run DSL.
                      config/wolf-risk_on-config.json -> wolf_risk_on_signals
  WOLF_LEG=risk_off -> Risk-Off book. Fires ONLY when the regime confirms
                      RISK-OFF (the mirror). Goes LONG defensives (gold / oil /
                      dollar / yen) and SHORT risk assets. Tighter DSL —
                      risk-off moves are sharp and reverse fast on a headline.
                      config/wolf-risk_off-config.json -> wolf_risk_off_signals

The edge is TRADING THE TURN: a single market-wide regime score is computed
from cross-asset confirmation (no single asset can flip the book), and the book
rotates to the prevailing regime. NOT a fixed bet (that is a Thesis Fund) and
NOT per-asset trend (that is Elephant) — Wolf's edge is the regime TRANSITION
itself, detected across the whole macro complex.
...
recent-signals race-dedup cache. The producer (wolf-producer.py) owns the
regime detector + scoring + emit. The runtime owns the LLM gate, DSL exits, and
...
# ─── Leg resolution ──────────────────────────────────────────
# One script, two books. WOLF_LEG selects the config file, the scanner
# name, the wallet env var, and the recent-signals cache.
LEG = (os.environ.get("WOLF_LEG") or "risk_on").strip().lower()
if LEG not in ("risk_on", "risk_off"):
    raise RuntimeError(
        f"WOLF_LEG must be 'risk_on' or 'risk_off' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "wolf-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"wolf-{LEG}-config.json"
...
# ${WOLF_RISK_ON_WALLET} / ${WOLF_RISK_OFF_WALLET}; the producer reads the
# same names so producer and runtime always agree on the wallet.
_WALLET_ENV = "WOLF_RISK_ON_WALLET" if LEG == "risk_on" else "WOLF_RISK_OFF_WALLET"
_STRATEGY_ENV = "WOLF_RISK_ON_STRATEGY_ID" if LEG == "risk_on" else "WOLF_RISK_OFF_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Wolf's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("wolf_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    ...
    try:
        ...
        sys.stderr.write(
            f"[wolf-v1:{LEG}] mcp_call_failed tool={tool} "
...
def log(msg):
    print(f"[wolf-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
