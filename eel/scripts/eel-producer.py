#!/usr/bin/env python3
# Senpi EEL Producer v1.0.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under Apache-2.0
# Source: https://github.com/Senpi-ai/senpi-skills
"""EEL v1.0.0 Producer — Electrons vs Hydrocarbons (energy-pair long/short).

Eel bets that AI datacenters are a structural new source of ELECTRICITY demand:
the power complex (uranium / gas / grid / fuel cells) outperforms crude oil, the
legacy transport fuel barely levered to AI. ONE producer script serves both
books; the EEL_LEG env var selects which:

  EEL_LEG=long   "Power" — LONG the AI-power complex on Hyperliquid XYZ: URNM
                  (uranium), NATGAS (gas-fired power), COPPER (grid), BE (fuel
                  cells), USAR (rare earth), trend-confirmed.
  EEL_LEG=short  "Oil" — SHORT crude oil (BRENTOIL + CL/WTI) — the legacy
                  hydrocarbon, trend-confirmed (capitulation guard).

Both legs are ENERGY, so an energy-wide move washes out and the P&L is the
electrons-vs-hydrocarbons spread — a sector-NEUTRAL pair, not an energy-direction
bet. Each book scores its universe on ABSOLUTE trend confirmation, with
cross-sectional relative strength as a tiebreaker. Per-group SIZING WEIGHTS
express conviction (uranium/copper big, fuel-cells small) without hardcoding
dollar amounts — every size scales with account value.

The long/short balance is an explicit operator decision — set by the power/oil
wallet funding split (see config `_hedge_note`); the default is a slight
long-power tilt.
...
Environment / config resolution:
  EEL_LEG            — REQUIRED. "long" or "short".
  SENPI_AUTH_TOKEN    — REQUIRED. Bearer token for MCP + signal POST.
  EEL_LONG_WALLET    — long-book strategy wallet (or config.wallet)
  EEL_SHORT_WALLET   — short-book strategy wallet (or config.wallet)
  EEL_DECISION_MODEL — bare LLM model name; resolved into runtime.yaml
...
SCANNER_NAME = f"eel_{LEG}_signals"
SIGNAL_TYPE = "EEL_POWER_LONG" if LEG == "long" else "EEL_OIL_SHORT"
DIRECTION = "LONG" if LEG == "long" else "SHORT"

NORM_DIV = 9.0

# ── Curated thematic universes (config.universe overrides) — an ENERGY pair ──
# LONG = the AI-power complex (uranium / gas-fired power / grid / fuel-cells /
#        rare-earth) — the electrons AI datacenters need.
# SHORT = crude oil — the legacy transport fuel losing share to electrification.
# Both legs are "energy," so an energy-wide move washes out and the P&L is the
# electrons-vs-hydrocarbons spread. Names not live on the board are skipped.
_HAVES = [
    "xyz:URNM",    # uranium miners — nuclear power for AI datacenters
    "xyz:NATGAS",  # gas-fired power (the bridge fuel for datacenter load)
    "xyz:COPPER",  # the grid / electrification metal
    "xyz:BE",      # Bloom Energy — on-site fuel cells for datacenters
    "xyz:USAR",    # USA Rare Earth — magnets / grid hardware
]
_HAVE_NOTS = [
...
_HAVE_NOTS_WEIGHTS = {"_default": 1.0}   # both crude benchmarks equal-weight
...
# ═══════════════════════════════════════════════════════════════
#
# Unlike a pure cross-sectional book (Cougar), Eel's universe is THEMATIC: every
...
if __name__ == "__main__":
    ...
    _kwargs = {
        ...
        "name": f"eel-{LEG}-producer-{_lock_id}",
