"""PIRANHA v1.0.0 — Shared config + MCP shim + helpers wrapper.

Liquidation-Cascade / Forced-Flow Hunter. Multi-asset whitelist
(BTC/ETH/SOL/HYPE). When open interest is unwinding fast (positions being
force-closed / liquidated) AND price is moving violently in one direction,
that's forced flow — liquidations begetting liquidations. Piranha rides it:
OI down + price spiking up = shorts squeezed → LONG; OI down + price
dropping = longs liquidated → SHORT. A thin order book on the side price
runs into confirms little resistance is left.

Architecture: helpers-native producer + senpi-trading-runtime LLM gate +
wide DSL ladder with a 24h hard_timeout (forced flow is short-horizon).
Carries the OI-state cache (read_oi_state/record_oi) to self-compute OI
velocity when market_get_asset_data's oi_velocity object is null.
"""
...
SKILL_DIR = Path(WORKSPACE) / "skills" / "piranha-strategy"
CONFIG_PATH = SKILL_DIR / "config" / "piranha-config.json"
...
# Race-window dedup TTL. Piranha ticks every 300s; ALO fills typically
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Piranha's MCP calls and signal "
            "POST both require it. Set it on the runtime host before "
            "starting the producer daemon."
        )
    client = SenpiClient()
    log_event("piranha_wrapper_enabled", sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    ...
    try:
        ...
        sys.stderr.write(
            f"[senpi_helpers] piranha_mcp_call_failed tool={tool} "
...
# it is, Piranha falls back to a self-computed OI delta: it persists the
...
def log(msg):
    print(f"[piranha-v1] {msg}", file=sys.stderr, flush=True)
