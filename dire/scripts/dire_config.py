"""DIRE v2.0.0 — Shared config + MCP shim + helpers wrapper.
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Dire's MCP calls and signal "
            "POST both require it. Set it on the runtime host before "
            "starting the producer daemon."
        )
    client = SenpiClient()
    log_event("dire_wrapper_enabled", sdk_path=_sdk_path)
    return client
...
def mcporter_call(tool, retries=2, timeout=30, **params):
    ...
    try:
        ...
        sys.stderr.write(
            f"[senpi_helpers] dire_mcp_call_failed tool={tool} "
...
def log(msg):
    print(f"[DIRE-v2] {msg}", file=sys.stderr)
