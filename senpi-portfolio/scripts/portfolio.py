...
def fetch_strategies(client, meta):
    ...
    if not_running:
        ...
        meta.setdefault("warnings", []).append(
            ...
            f"Confirm + fix with `openclaw senpi composer status <target> --json` (names the broken "
            f"lifecycle step + next verb; then composer update/close → redeploy).")
    ...
    if degraded:
        ...
        meta.setdefault("warnings", []).append(
            ...
            f"registered but not working cleanly. Confirm the cause with `openclaw senpi composer "
            f"status <target> --json` + the runtime logs (scanner registered? ticked? erroring?): "
