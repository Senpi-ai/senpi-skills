...
2. **The trader (OG)** — **senpi-trader-research** `--trader 0x…`: their `current_positions` (with
   `moved_from_entry_pct`), `momentum` (hot/cold — a proxy for "are they still active"), and `mirrorability`.
...
- **"Is my trader still active? No trades today."** — the OG's `momentum` is `cold`/`flat` and their book is
  unchanged since your last check → **the trader is idle, not a bug.** A mirror only fires when the OG does;
