...
If the user asks what to deploy, fetch the leaderboard. `min_budget` is a **suggested comfortable size, never a hard gate** — position size scales with budget via `margin_pct`, so any balance at or above the ~$100 platform floor can deploy any strategy (smaller budget = smaller positions, same logic). Recommend based on balance, but **never refuse a willing user over budget** — offer to start small or watch first instead:

| Balance | Recommended | Why |
|---|---|---|
| < $100 | Fund to ~$100 first, then any strategy | Below the Hyperliquid minimum to open a position at all |
| $100–$500 | Any strategy — start small; lead with the current #1 by ROE | Positions are smaller but the logic runs identically; offer "start small or watch first" |
| $500–$2,000 | Any strategy; comfortable sizing for multi-asset agents | Mid-range — most agents run at their suggested size |
| > $2,000 | Any strategy, or run several in parallel | Room for full sizing or multi-position deployments |
