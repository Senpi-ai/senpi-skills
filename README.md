...
| [hydra](hydra/) | Single-Coin Portfolio | Thesis + dip-buyer + stress hedge on ONE coin | A complete book on one major: a **thesis** head (trend-momentum, long/short) + a **dip** head (buys pullbacks in an uptrend) + a stress-gated **short hedge** — each its own wallet, regime-gated so they never fight. Net-long the coin, pressed on dips, cushioned on breaks. One engine, deploy per coin (**ETH / SOL / HYPE**) = 3 wallets each. The portfolio/hedge-pairing pattern, packaged. |
