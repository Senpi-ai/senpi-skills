...
| [hydra-eth](hydra/) | Single-Coin Portfolio (ETH) | Thesis + dip-buyer + stress hedge on ETH | A complete book on **ETH**: a **thesis** head (trend-momentum, long/short) + a **dip** head (buys pullbacks in an uptrend) + a stress-gated **short hedge** — each its own wallet, regime-gated so they never fight. Net-long ETH, pressed on dips, cushioned on breaks. 3 wallets, one shared engine. Funding split default 50/25/25. |
| [hydra-sol](hydra/) | Single-Coin Portfolio (SOL) | Thesis + dip-buyer + stress hedge on SOL | Same three-head book on **SOL**. Hedge stress threshold widened vs ETH (SOL is more volatile). 3 wallets, one shared engine. |
| [hydra-hype](hydra/) | Single-Coin Portfolio (HYPE) | Thesis + dip-buyer + stress hedge on HYPE | Same three-head book on **HYPE**. Hedge stress threshold is the widest so it doesn't arm on HYPE's normal 8%+ swings. 3 wallets, one shared engine. ⚠️ Aggressive (HYPE vol). |
