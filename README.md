...
| [eel](eel/) | Energy Pair (AI power) | Long power complex, short crude oil | Bets the **AI electricity-demand crunch** favors power over oil: longs the **AI-power complex** (uranium, gas, grid copper, fuel cells, rare-earth) and shorts **crude** (Brent + WTI). Both legs are energy, so an energy-wide move washes out — the cleanest-hedged thesis fund (an energy-sector-neutral pair). Conviction-sized, trend-confirmed. |
| [mongoose](mongoose/) | On-Chain Finance vs Legacy | Long crypto-finance rails, short legacy | Bets **money is migrating on-chain**: longs the **on-chain financial rails** (HYPE, Circle/CRCL stablecoins, Coinbase, Robinhood, crypto treasuries) and shorts **legacy finance + the index** (Blackstone + SP500). The P&L is the disruptor-vs-incumbent spread. Note: the hedge is cross-sector + thin (index-led) — a looser hedge; the long book is the strong side. |
| [boar](boar/) | Hard Money vs Paper | Long metals + BTC, short paper claims | Bets on **currency debasement**: longs **hard money** (gold, BTC, silver, platinum, palladium) and shorts the **paper economy** (SP500 + rate-sensitive growth RIVN/DKNG/HIMS). The P&L is the real-assets-beat-paper-claims spread. ⚠️ The **loosest hedge** of the family — in a melt-up gold + stocks rise together; the rate-sensitive short tilt tightens it but it's best as a *tilt*, not market-neutral. |
| [hydra-eth](hydra/) | Single-Coin Conviction (ETH) | Long-term ETH hold + cross-asset hedge | **v2.0:** a long-term conviction long on **ETH** (core + dip, ridden on a loose catastrophic-only DSL — not shaken out by normal vol), cushioned by a **cross-asset hedge** that shorts a diversified blend of *other* breaking assets (vol-parity sized, scaled up when ETH is breaking) — never shorts ETH. 3 wallets, one shared engine. Funding split default 50/25/25. |
| [hydra-sol](hydra/) | Single-Coin Conviction (SOL) | Long-term SOL hold + cross-asset hedge | Same v2.0 structure on **SOL** — long-term conviction long + a cross-asset short hedge of *other* breaking assets. 3 wallets, one shared engine. |
| [hydra-hype](hydra/) | Single-Coin Conviction (HYPE) | Long-term HYPE hold + cross-asset hedge | Same v2.0 structure on **HYPE** — long-term conviction long + a cross-asset short hedge of *other* breaking assets. 3 wallets, one shared engine. ⚠️ Aggressive (HYPE vol). |
| [caribou](caribou/) | Cross-Asset Trend Fund (Managed Futures) | Trend-follow **every asset class**, long+short, vol-parity | Managed futures across **all of Hyperliquid** — crypto, xyz stocks, indices, metals, energy. Long the uptrends + short the downtrends (each asset vs its own trend), sized to **equal risk** (volatility parity) and capped per asset class. Two independent sleeves on separate wallets — can hold the same asset long in one, short in the other. Net beta ~0, **crisis-positive** (shorts the fallers when markets break). The diversified version of a single-asset trend trader. |
| [whalehunter](whalehunter/) | Patient-Whale Conviction Copy | Copy the *one big bet* of consistent+patient winners | Shadows traders tagged **ELITE** (consistency) + **PATIENT** (activity) on Senpi Discover — winners who rarely trade — and strikes only when one opens a **new position that's a large % of their own balance** (their highest-conviction read), then mirrors it and **rides it wide**. Two independent sleeves on separate wallets → can hold the same asset long (one whale) and short (another) at once. Funding 50/50 — balanced long/short, no directional bias. ⚠️ Needs a USER-scoped token (Discover). |
