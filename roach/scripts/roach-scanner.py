#!/usr/bin/env python3
# Senpi ROACH Scanner v1.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under MIT
# Source: https://github.com/Senpi-ai/senpi-skills
"""ROACH v1.0 — Striker-Only Scanner. Stalker disabled.

ROACH tests the most extreme version of Fox's recommendation: disable Stalker
entirely and only trade STRIKER signals (first jumps backed by 1.5x volume
explosions). The hypothesis is that Stalker is pure drag — 17 trades at score
6-7 with 17.6% win rate and -$91.32 net. The one Striker signal (ZEC LONG
score 11) was the only explosive profile worth trading.

Cockroaches survive anything. ROACH survives by not trading when there's no
explosion. It will have long stretches of silence — Striker signals are rare
(maybe 1-3/day in active markets, 0 in chop). That patience IS the edge.

Single entry mode:
  STRIKER: Violent FIRST_JUMP + volume >= 1.5x. Score 9+, min 4 reasons.

Stalker detection code is still present (for scan history building) but its
signals are never emitted. The output always has stalkerDisabled: true and
stalkerSignals: [].

All hardened gates preserved:
- DSL state template in scanner output
- XYZ banned, leverage 7-10x, max 3 positions
- 2-hour per-asset cooldown, stagnation TP mandatory
...
def build_dsl_state_template(signal):
    ...
    return {
        ...
        "_roach_version": "1.0",
        "_note": "Generated by roach-scanner.py v1.2. Do not modify. Do not merge with dsl-profile.json.",
    }
