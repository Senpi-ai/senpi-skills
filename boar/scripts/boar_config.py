"""BOAR v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

BOAR — Hard Money vs Paper (Debasement) Hedge Fund. ONE producer script driven
by the BOAR_LEG env var into one of two books, each bound to its own wallet +
runtime + DSL:

  BOAR_LEG=long  -> "Hard money" book. LONGS precious metals + BTC: GOLD, BTC
                    (digital gold), SILVER, PLATINUM, PALLADIUM, trend-confirmed.
                    config/boar-long-config.json -> boar_long_signals
  BOAR_LEG=short -> "Paper" book. SHORTS the broad market + rate-sensitive
                    long-duration growth — SP500 + RIVN/DKNG/HIMS —
                    trend-confirmed (capitulation guard).
                    config/boar-short-config.json -> boar_short_signals

The thesis: fiscal dominance / currency debasement — scarce real assets
outperform fiat-denominated financial claims as deficits compound. LONG hard
money, SHORT paper — the P&L is the real-vs-paper spread. HONEST CAVEAT: the
loosest hedge of the family — in a pure liquidity melt-up gold AND stocks can
rise together (positive correlation); the short tilts to rate-sensitive names to
tighten it. The long/short balance is an explicit operator decision (see config
`_hedge_note`), defaulting to a net-long-hard-money tilt. NOT a copy-trader.
...
race-dedup cache. The producer (boar-producer.py) owns the thematic universe
...
# ─── Leg resolution ──────────────────────────────────────────
LEG = (os.environ.get("BOAR_LEG") or "long").strip().lower()
if LEG not in ("long", "short"):
    raise RuntimeError(
        f"BOAR_LEG must be 'long' or 'short' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "boar-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"boar-{LEG}-config.json"
STATE_DIR = SKILL_DIR / "state"
RECENT_SIGNALS_PATH = STATE_DIR / f"recent-signals-{LEG}.json"

STATE_DIR.mkdir(parents=True, exist_ok=True)

_WALLET_ENV = "BOAR_LONG_WALLET" if LEG == "long" else "BOAR_SHORT_WALLET"
_STRATEGY_ENV = "BOAR_LONG_STRATEGY_ID" if LEG == "long" else "BOAR_SHORT_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Boar's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("boar_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    try:
        ...
        sys.stderr.write(
            f"[boar-v1:{LEG}] mcp_call_failed tool={tool} "
...
def get_positions(wallet):
    ...
    Boar holds both xyz equities and main-DEX crypto on the same wallet, so
...
def log(msg):
    print(f"[boar-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
