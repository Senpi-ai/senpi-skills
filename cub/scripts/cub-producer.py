#!/usr/bin/env python3
# Senpi CUB Producer v1.0.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under Apache-2.0
# Source: https://github.com/Senpi-ai/senpi-skills
"""CUB v1.0.0 Producer — Two-Speed-Market (K-shaped) cross-asset long/short.

Cub is a variation of LION: it allocates ~90% to the Lion two-speed AI long/short
engine and ~10% to a pre-IPO ramp satellite. ONE producer script serves all three
books; the CUB_LEG env var selects which:

  CUB_LEG=long    "Haves" (part of the ~90%) — LONG the AI complex on Hyperliquid
                  XYZ (NVDA, AMD, MRVL, TSM, ASML, ARM, AVGO, CRWV, PLTR, ORCL, …)
                  + the crypto winners (HYPE large, SOL modest), trend-confirmed.
  CUB_LEG=short   "Have-nots" (part of the ~90%) — SHORT the broad U.S. market via
                  the SP500 index (the "economy suffers" core) + a curated, gated
                  basket of laggard crypto alts, trend-confirmed (capitulation guard).
  CUB_LEG=preipo  "Pre-IPO ramp" (~10%) — auto-DISCOVER pre-IPO perpetuals (IPOPs)
                  by their funding signature (Lemur method) and LONG the ones
                  ramping into their listing (the SpaceX / Cerebras pattern). No
                  static universe; episodic (most ticks find 0-2 IPOPs).

The long/short books score a curated CROSS-ASSET universe (xyz equities + main-DEX
crypto on one cross-margined wallet); the preipo book scores a DISCOVERED IPOP
universe. All on ABSOLUTE trend confirmation, with cross-sectional relative
strength as a tiebreaker. Per-group SIZING WEIGHTS express conviction (HYPE large,
SOL modest, SP500 core) without hardcoding dollar amounts — every size scales with
account value. The edge is the K-shaped DISPERSION (90%) plus pre-IPO optionality
(10%), not market direction.

Allocation (90/10) and net exposure are explicit operator decisions — set by how
much capital is funded into each of the three wallets (plus the per-leg slots /
margin / sizing knobs); not hardcoded. See each config's _alloc_note /
_net_exposure_note. Default: ~90% across long+short (Lion net-long tilt), ~10% preipo.
...
Environment / config resolution:
  CUB_LEG            — REQUIRED. "long" or "short".
  SENPI_AUTH_TOKEN    — REQUIRED. Bearer token for MCP + signal POST.
  CUB_LONG_WALLET    — long-book strategy wallet (or config.wallet)
  CUB_SHORT_WALLET   — short-book strategy wallet (or config.wallet)
  CUB_DECISION_MODEL — bare LLM model name; resolved into runtime.yaml
...
import cub_config as cfg

from senpi_runtime_helpers import SenpiClientError, producer_daemon  # type: ignore  # noqa: E402


VERSION = "1.0.0"
LEG = cfg.LEG  # "long" (haves) | "short" (have-nots) | "preipo" (pre-IPO ramp)
SCANNER_NAME = f"cub_{LEG}_signals"
SIGNAL_TYPE = {"long": "CUB_HAVES_LONG", "short": "CUB_HAVENOTS_SHORT",
               "preipo": "CUB_PREIPO_LONG"}[LEG]
DIRECTION = "SHORT" if LEG == "short" else "LONG"   # long + preipo are LONG books
IS_PREIPO = (LEG == "preipo")
...
_DEFAULTS = {
    ...
    "preipo": {
        # pre-IPO ramp sleeve (Lemur IPOP-discovery method) — the 10% satellite.
        # Universe is DISCOVERED each tick by funding signature (no static
        # whitelist); LONG-only (ride the pre-listing ramp).
        "sizingWeights": {"_default": 1.0},
        "minScore": 4,                 # pre-IPO data is sparse — slightly lower bar
        "marginPct": 0.15,
        "maxLeverage": 5,              # IPOPs are venue-capped at 5x anyway
        "maxSlots": 3,
        "venueMinNotionalUsd": 10,
        "minNotionalPctOfEquity": 0.01,
        "tickSeconds": 300,
        "ipopFundingMaxAbs": 1e-7,     # IPOP funding signature (throttled pre-listing)
        "ipopMaxLeverageCap": 5,       # IPOP leverage signature
        "ipopLiqVolMultiple": 30,      # budget-relative IPOP liquidity floor (no $ floor)
        "rankPoolSize": 16,
        "rsThresholdPct": 3.0,
...
# ═══════════════════════════════════════════════════════════════
#
# Unlike a pure cross-sectional book (Cougar), Cub's universe is THEMATIC: every
...
# ═══════════════════════════════════════════════════════════════
# Universe builders — curated whitelist (haves/have-nots) OR discovered IPOPs
# ═══════════════════════════════════════════════════════════════

def ipop_universe(config, meta_map, canonical, min_day_vol):
    """Lemur-method pre-IPO discovery (the preipo leg). An IPOP carries a
    structural funding signature: |funding| <= ipopFundingMaxAbs AND venue
    max_leverage <= ipopMaxLeverageCap (the throttled pre-listing state — once it
    IPOs and converts to a standard equity, funding jumps ~100x and the leverage
    cap lifts). Returns the live xyz IPOP names whose 24h volume clears a
    BUDGET-RELATIVE floor (min_day_vol = ipopLiqVolMultiple x position notional —
    NO hardcoded $; the IPOP universe is too small for a median gate). LONG-only:
    the absolute-trend gate inside score_thematic longs only the ones ramping."""
    max_funding = float(config.get("ipopFundingMaxAbs", _DEFAULTS["ipopFundingMaxAbs"]))
    lev_cap = int(config.get("ipopMaxLeverageCap", _DEFAULTS["ipopMaxLeverageCap"]))
    out = []
    for name in canonical:
        ...
        try:
            funding_abs = abs(float(ctx.get("funding", 0) or 0))
        except (TypeError, ValueError):
            continue
...
def main():
    ...
    if IS_PREIPO:
        # Lemur IPOP discovery with a budget-relative liquidity floor (no $ literal)
        liq_mult = float(config.get("ipopLiqVolMultiple", _DEFAULTS["ipopLiqVolMultiple"]))
        min_day_vol = liq_mult * (account_value * float(margin_pct) * float(max_lev))
        universe = ipop_universe(config, meta_map, canonical, min_day_vol)
    else:
    ...
    if len(rs) < (1 if IS_PREIPO else 2):
        cfg.output({"status": "ok", "leg": LEG, "scanned": len(universe),
                    "candidates": 0, "signals_pushed": 0,
                    "note": ("WAITING — no live IPOPs ramping" if IS_PREIPO
                             else "WAITING — thematic universe too thin to evaluate"),
                    "elapsed_sec": round(time.time() - run_start, 2),
                    "_cub_producer_version": VERSION})
        return

    mean_rs = sum(r[1] for r in rs) / len(rs)
    rs.sort(key=lambda x: x[1], reverse=(DIRECTION == "LONG"))
    pool = rs[:rank_pool]
...
if __name__ == "__main__":
    ...
    _kwargs = {
        ...
        "name": f"cub-{LEG}-producer-{_lock_id}",
