"""RAPTOR v4.0.0 — Shared config + MCP shim + helpers wrapper.
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Raptor's MCP calls and signal "
            "POST both require it."
        )
    client = SenpiClient()
    log_event("raptor_wrapper_enabled", sdk_path=_sdk_path)
    return client
...
def get_wallet_and_strategy():
    wallet = os.environ.get("RAPTOR_WALLET", "").strip()
    strategy_id = os.environ.get("RAPTOR_STRATEGY_ID", "").strip()
...
def mcp_call(tool, **params):
    try:
        ...
        log_event("raptor_mcp_call_failed", tool=tool, error=str(e))
        return None
...
def log(msg):
    print(f"[raptor-v4] {msg}", file=sys.stderr, flush=True)
