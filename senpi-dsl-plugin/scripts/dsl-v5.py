...
def archive_state_file(state_file: str, state_dir: str, strategy_id: str) -> None:
    """Move state_file to {stateDir}/archive/{strategyId}/{stem}-{utc_ts}.json instead of deleting."""
    archive_dir = os.path.join(state_dir, "archive", strategy_id)
    os.makedirs(archive_dir, exist_ok=True)
    stem = os.path.splitext(os.path.basename(state_file))[0]
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    archive_path = os.path.join(archive_dir, f"{stem}-{ts}.json")
...
def cleanup_strategy_state_dir(state_dir: str, strategy_id: str) -> int:
    """Archive all .json state files in strategy dir. Return count archived."""
    deleted = 0
    strategy_dir = os.path.join(state_dir, strategy_id)
    if not os.path.isdir(strategy_dir):
        return 0
    for name in os.listdir(strategy_dir):
        ...
        if name.endswith(".json") and os.path.isfile(path):
            try:
                archive_state_file(path, state_dir, strategy_id)
                deleted += 1
            except OSError:
                pass
    return deleted
...
def save_or_delete_state(
    ...
    if closed:
        try:
            _state_dir = os.path.dirname(os.path.dirname(state_file))
            _strategy_id = os.path.basename(os.path.dirname(state_file))
            archive_state_file(state_file, _state_dir, _strategy_id)
            return close_result
        except OSError as e:
            ...
            try:
                ...
                pass
            return (close_result or "") + f"; archive_failed: {e}"
    with open(state_file, "w") as f:
        json.dump(state, f, indent=2)
    return close_result
...
def cmd_add_dsl() -> None:
    ...
    parser.add_argument("--leverage", type=float, default=1.0, help="Leverage (positive). Default 1. Optional; only needed if you want ROE/tiers to use a different leverage than implied from position.")
    parser.add_argument("--margin", type=float, default=None, help="Position margin (collateral) in quote units for ROE. Optional; defaults to entry*size/leverage from position.")
    ...
    if entry_price <= 0 or size <= 0:
        ...
        sys.exit(1)
    leverage = max(1.0, args.leverage)
    margin = args.margin
    if margin is None or margin <= 0:
        margin = entry_price * size / leverage
    if margin <= 0:
        print(json.dumps({"action": "add-dsl", "status": "error", "error": "invalid_margin", "message": "margin must be > 0 (or omit to derive from position)"}))
        sys.exit(1)
    state = _add_dsl_build_state(
        asset, args.direction.upper(), leverage, margin,
