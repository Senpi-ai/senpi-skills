...
| state | meaning |
|---|---|
| `healthy` | Daemon running, recent tick, last status `ok` **or `skipped_locked`** (a benign overlap from a still-running prior tick — counted as healthy) |
...
| `last_tick_failed` | Running, recent tick, but `last_tick_status` is **`error` or `timeout`**. (`skipped_locked` is NOT failed — it's healthy, see above.) |
