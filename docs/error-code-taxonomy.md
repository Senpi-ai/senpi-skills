...
say) — but `belowMin` is a fact about the BUDGET, not about which code led, so it is set whenever
the budget is under the figure, under either code. Do not read the absence of
