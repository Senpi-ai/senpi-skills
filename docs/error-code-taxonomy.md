...
The two `W_BUDGET_*` rows are the only **advisory** codes in this table, and the `W_` prefix is how
...
`minBudgetUnresolved` (and print as `calculated minimum:` / `warn:` lines under `senpi deploy
status`), never as a `refused` step.
