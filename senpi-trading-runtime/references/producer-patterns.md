...
| Agent | Version | Asset / Universe | Description | Tags |
|---|---|---|---|---|
| **Hydra-ETH / Hydra-SOL / Hydra-HYPE** | v2.0 | one major per variant (ETH / SOL / HYPE; extensible), three heads each: core (long-term thesis, long/short) + dip (uptrend pullback-buyer, long) + hedge (**cross-asset blend short**) | **Single-Coin Conviction Fund with a cross-asset hedge.** Named per-coin variants over ONE shared producer + runtimes; each = three wallets. **v2.0:** core/dip are a long-term thesis HOLD on a catastrophic-only DSL (not shaken out by normal vol); the hedge shorts a diversified BLEND of OTHER assets actually breaking down (vol-parity sized, scaled up by thesis stress) — never the thesis coin. Cushions market risk without betting against the thesis; auto-scales (light in calm, heavy in a selloff). core 5x / dip 4x / hedge 3x; funding split default 50/25/25. v1.0 was a same-asset stress short; SM confirm + correlation-aware blend weighting planned v2.1. | Hedge-fund, Single-coin-conviction, Cross-asset-hedge, Long-term-hold, Vol-parity, Per-coin-variant, Three-wallet |
