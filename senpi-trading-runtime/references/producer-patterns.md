...
| **Boar** | v1.0 | hard money — GOLD, BTC (digital gold), SILVER, PLATINUM, PALLADIUM | paper — SP500 + rate-sensitive long-duration growth (RIVN/DKNG/HIMS) | **cross-sector, melt-up-correlated** — loosest; rate-sensitive short tilt tightens it; documented honestly | Hedge-fund, Thesis-fund, Debasement, Hard-money, Gold, BTC, Two-wallet |
...
core:  trend(coin) -> LONG if 4h bullish / SHORT if bearish      # the thesis, always-on
dip:   if 4h bullish and pulled_back(coin): LONG                  # complement, UPTRENDS ONLY
hedge: if 4h bearish and fast_drawdown(coin): SHORT (capit-guard) # cushion, DOWNTRENDS/STRESS ONLY
# regime lanes: uptrend -> core long + dip; downtrend -> core short + hedge; never opposing
```
...
| Agent | Version | Asset / Universe | Description | Tags |
|---|---|---|---|---|
| **Hydra-ETH / Hydra-SOL / Hydra-HYPE** | v2.1 | one major per variant (ETH / SOL / HYPE; extensible), three heads each: core (long-term thesis, long/short) + dip (uptrend pullback-buyer, long) + hedge (**cross-asset blend short**, optionally hybrid) | **Single-Coin Conviction Fund with a cross-asset hedge.** Named per-coin variants over ONE shared producer + runtimes; each = three wallets. core/dip are a long-term thesis HOLD on a catastrophic-only DSL; the hedge shorts a diversified BLEND of OTHER assets breaking down (vol-parity, thesis-stress-scaled). **v2.1:** a **1d-alignment chop filter** on core/dip (only enter when the daily confirms the 4h — stops buying the range-top in chop) + deeper dip pullback; and an opt-in **hybrid hedge** (`hedgeIncludesThesis`, on for HYPE) that ALSO shorts the thesis coin when IT is breaking down, cushioning idiosyncratic moves the cross-asset blend misses. core 5x / dip 4x / hedge 3x; funding 50/25/25. **Deploy note:** the loose core DSL is a runtime file — recreate the core runtime on upgrade or it keeps the prior tighter DSL. | Hedge-fund, Single-coin-conviction, Cross-asset-hedge, Chop-filter, Hybrid-hedge, Per-coin-variant, Three-wallet |
