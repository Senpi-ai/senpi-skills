...
| Command | What it does | Options |
|---|---|---|
| `runtime create` | **Internal — not the deploy path.** Adds a runtime from a YAML file (or pasted YAML) and hot-loads it, skipping the funds preflight, the attribution and the verified tick that `senpi deploy` performs. Pasted as content (`-c`) with no `--runtime-yaml-dir` it also leaves a relative scanner path nothing to resolve against and the install gate refuses `[E_VALIDATE_UNRESOLVABLE_SCANNER_PATH]` — `-p` derives that directory from the file, so only the content form breaks. | `-p, --path <path>` (path to the runtime.yaml) · `-c, --content <yaml>` (paste YAML directly) · `--runtime-id <id>` (name; default derived from the file name/content) |
