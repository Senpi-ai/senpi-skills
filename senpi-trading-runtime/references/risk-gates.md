...
> ⚠️ **Multi-wallet funds — do NOT set `daily_loss_limit_pct` (until the engine fix).** The Daily-Loss-Halt gate computes its limit as `pct × today_snapshot.account_value.at_day_open`, where `at_day_open` comes from Hyperliquid's **perpDay** endpoint — which reads **~$0** for these strategy wallets (perp account value is $0 for a leg holding only idle cash / before it deploys margin, and the perpDay base does not reliably reset). So the limit computes to **`$0.00`**, and **any** real loss on the wallet breaches it → the book is **permanently halted** (`"down $X, limit $0.00"`, re-tripping every UTC day; the daily reset doesn't help because the base stays $0). Observed live across Wolf, Hydra, and Mongoose — Mongoose's long book hit it on its *own* ~$62 loss against a $0 limit. Until the runtime computes the limit base **per strategy-wallet from total account value**, leave this gate OFF on multi-wallet funds and rely on `drawdown_halt_pct` (PnL-based, immune to this) + the DSL `max_loss_pct` + cooldowns.
