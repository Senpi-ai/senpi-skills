...
- ✅ Prove it before you hand it over: `openclaw senpi validate <instance-dir>` must return **PASS**
  (the directory holding that instance's recipe — a multi-instance package root refuses
  `[E_VALIDATE_NO_RECIPE]`, and each instance needs its own pass).
