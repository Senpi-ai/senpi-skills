...
`weak_peak_cut` / `dead_weight_cut` "have no phase guard and still fire"); the concepts reference
must not tell an author that entering Phase 2 disarms `hard_timeout`."""
import os
...
def test_hard_timeout_is_documented_as_either_phase():
    ...
    assert "entering Phase 2 does\n  not disarm `hard_timeout`" in text
