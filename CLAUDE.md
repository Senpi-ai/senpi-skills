...
- **Strategic** — "Help me pick a strategy", "What should I trade?", "Build a trading strategy", "Recommend a strategy". The user wants help deciding. **Template-first picker:** walk the Decision Tree in `senpi-trading-runtime/references/producer-patterns.md` (the **canonical routing source** — Layers 0–3), cross-reference `catalog.json` for install metadata (name, emoji, tagline, min_budget, install command), install via `install_skill`. **Fallback** (only if no template fits): author a new one via `senpi-trading-runtime/references/strategy-creation.md`. *Routing always goes through the Decision Tree; `catalog.json` is install/display metadata only — when the two disagree, the Decision Tree wins.*
