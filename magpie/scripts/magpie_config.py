"""MAGPIE v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

MAGPIE — IPO / New-Listing Event Hedge Fund. ONE producer script driven by the
MAGPIE_LEG env var into one of two books that together trade the full
pre-IPO → listing → graduation arc of tokenized equities on Hyperliquid XYZ
(trade.xyz IPOPs like SpaceX), each bound to its own wallet + runtime + DSL:

  MAGPIE_LEG=pre_listing -> Pre-listing accumulation book. Auto-discovers IPOPs
                          (pre-IPO perpetuals) by their funding signature (Lemur
                          method: |funding| <= ~1e-7, max_leverage <= 5, min
                          volume) and rides the pre-listing trend into the IPO,
                          trend + SM confirmed. Moderate DSL (Discovery Bounds
                          throttle IPOP velocity). LONG/SHORT per trend.
                          config/magpie-pre_listing-config.json -> magpie_pre_listing_signals
  MAGPIE_LEG=graduation  -> Graduation-momentum book. Detects the IPOP→equity
                          CONVERSION (Falcon method: funding jumps ~100x, the
                          leverage cap lifts, the price throttle comes off) and
                          rides the explosive first-days price discovery — the
                          SpaceX $1.4B-day-1 pattern. Wide let-winners-run DSL.
                          config/magpie-graduation-config.json -> magpie_graduation_signals

The edge is EVENT alpha around new equity listings — distinct from a directional
view (Cougar trades the ongoing equity market). NOT a copy-trader. The
pre-listing book uses Lemur's IPOP-discovery; the graduation book uses Falcon's
class-state conversion detector (this module owns that persistence).

This module owns: leg resolution, config load, the senpi_runtime_helpers
client, the MCP call shim, account/position pulls, the recent-signals race-dedup
cache, AND the instrument-class + conversion-window caches (graduation book).
The producer (magpie-producer.py) owns scoring + emit. The runtime owns the LLM
...
# ─── Leg resolution ──────────────────────────────────────────
LEG = (os.environ.get("MAGPIE_LEG") or "pre_listing").strip().lower()
if LEG not in ("pre_listing", "graduation"):
    raise RuntimeError(
        f"MAGPIE_LEG must be 'pre_listing' or 'graduation' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "magpie-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"magpie-{LEG}-config.json"
STATE_DIR = SKILL_DIR / "state"
RECENT_SIGNALS_PATH = STATE_DIR / f"recent-signals-{LEG}.json"
# Graduation book only: class cache (IPOP vs STANDARD) + conversion-window cache.
CLASS_STATE_PATH = STATE_DIR / f"class-state-{LEG}.json"
CONVERSIONS_PATH = STATE_DIR / f"conversions-{LEG}.json"

STATE_DIR.mkdir(parents=True, exist_ok=True)

_WALLET_ENV = "MAGPIE_PRE_LISTING_WALLET" if LEG == "pre_listing" else "MAGPIE_GRADUATION_WALLET"
_STRATEGY_ENV = "MAGPIE_PRE_LISTING_STRATEGY_ID" if LEG == "pre_listing" else "MAGPIE_GRADUATION_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Magpie's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("magpie_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    try:
        ...
        sys.stderr.write(
            f"[magpie-v1:{LEG}] mcp_call_failed tool={tool} "
...
def get_positions(wallet):
    ...
    taken ONCE via max() across the two sections — never summed.
    """
...
# ─── Instrument-class cache (graduation book: detect IPOP→STANDARD) ──
#
# market_list_instruments exposes funding + max_leverage per xyz instrument.
# The graduation book classifies each into IPOP vs STANDARD by the funding
# signature, then compares against the prior-tick classification to detect the
# conversion flip. Stores {name: {"class": "IPOP"|"STANDARD", "ts": epoch}}.
...
#
# When a flip is detected the instrument is stamped here so it stays eligible to
# trade post-conversion momentum for conversionWindowHours, not just on the flip
# tick. Stores {name: detected_epoch}.
...
def log(msg):
    print(f"[magpie-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
