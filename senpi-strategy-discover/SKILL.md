...
> **Where the scripts live (read this before you run anything).** The `exec` tool's working directory is
> the workspace (`/data/workspace`), **not** this skill directory — so a workspace-relative
> `python3 scripts/discover.py` will fail with `No such file or directory`. The scripts live under
> `$OPENCLAW_STATE_DIR/skills/senpi-strategy-discover/scripts/` (e.g.
> `/data/.openclaw/skills/senpi-strategy-discover/scripts/` on Railway). **Always `cd` into the skill
> directory first** (`cd "$OPENCLAW_STATE_DIR/skills/senpi-strategy-discover"`), or call the script by
> its absolute path. **Never** use a bare workspace-relative path.
...
Invoke via the `exec` tool. **`cd` into the skill directory first** (see *Where the scripts live*
above) so the relative `scripts/discover.py` resolves. **Concrete flags only** — everything else is
your job:
...
If `OPENCLAW_STATE_DIR` is unset, fall back to the absolute path:
`python3 /data/.openclaw/skills/senpi-strategy-discover/scripts/discover.py …`

**Install note — ship the whole `scripts/` dir.** `discover.py` imports a vendored MCP helper,
`scripts/mcp_client.py`, at runtime. Installing `scripts/` partially (e.g. an installer that strips
`_`-prefixed or undeclared files) breaks it with `No module named 'mcp_client'`. Stdlib only, no other
...
3. **Size & lock in (Layer 3)** — ask budget (and risk if not implied) only to size + pick the DSL
   preset. Optionally run the engine with `--context-only` to reference holdings (confirm first; never silently
