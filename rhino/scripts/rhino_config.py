"""RHINO v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

RHINO — Tail-Risk / Crisis-Alpha Hedge Fund. ONE producer script driven by the
RHINO_LEG env var into one of two books, each bound to its own wallet + runtime
+ DSL. Rhino carries cheap convexity: it bleeds a little in calm and pays big
in shocks (geopolitics, macro stress, liquidation cascades).

  RHINO_LEG=hedge      -> Always-on Hedge book. A small, persistent long carry
                         in the crisis-beneficiary complex (gold / oil / dollar
                         / yen), entered only when a defensive is actually
                         trending up (no falling-knife hedges). This is the
                         standing insurance — small size, wide let-it-run DSL.
                         config/rhino-hedge-config.json -> rhino_hedge_signals
  RHINO_LEG=escalation -> Stress-gated Escalation book. Dormant in calm; fires
                         HARD only when the shared STRESS detector confirms a
                         shock (oil breaking out + equities breaking down + gold
                         bid + BTC rolling over + vol/range expansion). Goes
                         LONG the spiking crisis assets and SHORT the cratering
                         risk assets — the convex add. Moderate-tight DSL that
                         banks the spike (crises reverse violently).
                         config/rhino-escalation-config.json -> rhino_escalation_signals

The edge is CONVEXITY: own the crisis hedge cheaply before the shock, and add
leveraged exposure the moment stress confirms. The hedge book carries always;
the escalation book sits mostly in cash as dry powder. NOT a fixed bet and NOT
per-asset trend — Rhino is built around a stress regime.
...
recent-signals race-dedup cache. The producer (rhino-producer.py) owns the
stress detector + scoring + emit. The runtime owns the LLM gate, DSL exits, and
all risk.guard_rails. NOT a copy-trader — each book scores its own universe.
...
# ─── Leg resolution ──────────────────────────────────────────
# One script, two books. RHINO_LEG selects the config file, the scanner
# name, the wallet env var, and the recent-signals cache.
LEG = (os.environ.get("RHINO_LEG") or "hedge").strip().lower()
if LEG not in ("hedge", "escalation"):
    raise RuntimeError(
        f"RHINO_LEG must be 'hedge' or 'escalation' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "rhino-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"rhino-{LEG}-config.json"
...
# ${RHINO_HEDGE_WALLET} / ${RHINO_ESCALATION_WALLET}; the producer reads the
# same names so producer and runtime always agree on the wallet.
_WALLET_ENV = "RHINO_HEDGE_WALLET" if LEG == "hedge" else "RHINO_ESCALATION_WALLET"
_STRATEGY_ENV = "RHINO_HEDGE_STRATEGY_ID" if LEG == "hedge" else "RHINO_ESCALATION_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Rhino's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("rhino_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    ...
    try:
        ...
        sys.stderr.write(
            f"[rhino-v1:{LEG}] mcp_call_failed tool={tool} "
...
def log(msg):
    print(f"[rhino-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
