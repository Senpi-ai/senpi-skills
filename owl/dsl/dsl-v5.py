...
def normalize_state_phase_config(state: dict) -> bool:
    ...
    if "consecutiveBreachesRequired" not in p2:
        ...
        changed = True
    # Backfill highWaterPrice if missing (use entryPrice as default)
    if "highWaterPrice" not in state or state["highWaterPrice"] is None:
        state["highWaterPrice"] = state.get("entryPrice", 0.0)
        changed = True
    # Backfill other commonly missing fields
    if "currentBreachCount" not in state:
        state["currentBreachCount"] = 0
        changed = True
    if "tierFloorPrice" not in state:
        state["tierFloorPrice"] = None
        changed = True
    if "consecutiveFetchFailures" not in state:
...
def try_close_position(
    ...
    for attempt in range(close_retries):
        try:
            ...
            # Parse JSON response to check actual success (don't grep for "error" — all responses contain the key)
            close_success = False
            if cr.returncode == 0 and result_text:
                try:
                    resp = json.loads(result_text)
                    # Unwrap mcporter envelope: check data.success or top-level success
                    inner = resp.get("data", resp)
                    if isinstance(inner, dict) and inner.get("success") is True:
                        close_success = True
                    # Also treat CLOSE_NO_POSITION as success — position is already gone
                    err = inner.get("error") if isinstance(inner, dict) else None
                    if isinstance(err, dict) and err.get("code") == "CLOSE_NO_POSITION":
                        close_success = True
...
def main() -> None:
    ...
    # Safety: only reconcile if clearinghouse returned at least as many positions as state files.
    # strategy_get_clearinghouse_state can return incomplete data for xyz assets.
    state_files = list_strategy_state_files(state_dir, strategy_id)
    if len(coins) >= len(state_files):
        for path, asset in list(state_files):
        ...
        print(json.dumps({
            "status": "warning",
            "warning": "clearinghouse_incomplete",
            "strategy_id": strategy_id,
            "message": f"Clearinghouse returned {len(coins)} positions but {len(state_files)} state files exist. Skipping reconcile to avoid false archival.",
            "ch_coins": sorted(coins),
            "state_assets": sorted(a for _, a in state_files),
            "time": now,
        }))

    # Process all state files (not just clearinghouse coins) to handle incomplete clearinghouse
    processed = 0
    state_files = list_strategy_state_files(state_dir, strategy_id)
    for state_file, asset in state_files:
