"""LION v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

LION — Two-Speed-Market (K-Shaped) Cross-Asset Long/Short Hedge Fund. ONE
producer script driven by the LION_LEG env var into one of two thematic books,
each bound to its own wallet + runtime + DSL:

  LION_LEG=long  -> "Haves" book. LONGS the structural winners of a two-speed
                    ...
                    config/lion-long-config.json -> lion_long_signals
  LION_LEG=short -> "Have-nots" book. SHORTS the laggards — the broad U.S. equity
                    ...
                    config/lion-short-config.json -> lion_short_signals
...
race-dedup cache. The producer (lion-producer.py) owns the thematic universe
...
# ─── Leg resolution ──────────────────────────────────────────
LEG = (os.environ.get("LION_LEG") or "long").strip().lower()
if LEG not in ("long", "short"):
    raise RuntimeError(
        f"LION_LEG must be 'long' or 'short' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "lion-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"lion-{LEG}-config.json"
STATE_DIR = SKILL_DIR / "state"
RECENT_SIGNALS_PATH = STATE_DIR / f"recent-signals-{LEG}.json"

STATE_DIR.mkdir(parents=True, exist_ok=True)

_WALLET_ENV = "LION_LONG_WALLET" if LEG == "long" else "LION_SHORT_WALLET"
_STRATEGY_ENV = "LION_LONG_STRATEGY_ID" if LEG == "long" else "LION_SHORT_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Lion's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("lion_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    try:
        ...
        sys.stderr.write(
            f"[lion-v1:{LEG}] mcp_call_failed tool={tool} "
...
def get_positions(wallet):
    ...
    Lion holds both xyz equities and main-DEX crypto on the same wallet, so
...
def log(msg):
    print(f"[lion-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
