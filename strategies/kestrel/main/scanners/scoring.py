...
def score_breakout(asset, candles_1h, asset_context, order_book, sm_record, config=None):
    ...
    if abs(pct_1h) >= 3.0:
        ...
        reasons.append(f"massive 1h breakout {pct_1h:+.2f}%")
    elif abs(pct_1h) >= 2.0:
        score += 4  # v2-quirk: was 3 pre-v2.0
        reasons.append(f"strong 1h breakout {pct_1h:+.2f}%")
    elif abs(pct_1h) >= bt_1h:
        score += 3  # v2-quirk: was 2 pre-v2.0
        reasons.append(f"1h breakout {pct_1h:+.2f}%")
    ...
    if abs(pct_4h) >= bt_4h:
        if (breakout_dir == "LONG" and pct_4h > 0) or \
           (breakout_dir == "SHORT" and pct_4h < 0):
            score += 2
            reasons.append(f"4h trend confirms at {pct_4h:+.2f}%")
    elif abs(pct_4h) >= 1.0:
        if (breakout_dir == "LONG" and pct_4h > 0) or \
           (breakout_dir == "SHORT" and pct_4h < 0):
            score += 1
            reasons.append(f"4h trend aligned at {pct_4h:+.2f}%")
    ...
    if abs(pct_4h) >= exhaustion:
        if (breakout_dir == "LONG" and pct_4h > 0) or \
           (breakout_dir == "SHORT" and pct_4h < 0):
            score -= 2
            reasons.append(f"move may be exhausted after {pct_4h:+.2f}% in 4h")
    elif abs(pct_4h) >= tiring:
        if (breakout_dir == "LONG" and pct_4h > 0) or \
           (breakout_dir == "SHORT" and pct_4h < 0):
            score -= 1
            reasons.append(f"move tiring after {pct_4h:+.2f}% in 4h")
    ...
    if len(candles_1h) >= 4:
        ...
        if len(vols) >= 4 and vols[:-1] and sum(vols[:-1]) > 0:
            ...
            if vol_vs_avg >= 2.0:
                score += 2
                reasons.append(f"volume surged to {vol_vs_avg:.1f}x average")
            elif vol_vs_avg >= 1.3:
                score += 1
                reasons.append(f"volume up {vol_vs_avg:.1f}x average")
    ...
    if sm:
        ...
        if sm_dir == breakout_dir and sm_pct >= 3:
            score += 2
            reasons.append(f"smart money confirms: {sm_pct:.1f}% ({sm_traders} traders)")
        elif sm_dir == breakout_dir and sm_pct >= 1:
            score += 1
            reasons.append(f"smart money building: {sm_pct:.1f}% ({sm_traders} traders)")
        elif sm_dir != breakout_dir and sm_pct >= 5:
            score += 1
            reasons.append(f"smart money trapped {sm_dir.lower()} at {sm_pct:.1f}%")
    ...
    if isinstance(asset_ctx, dict):
        ...
        if (breakout_dir == "LONG" and funding < -0.001) or \
           (breakout_dir == "SHORT" and funding > 0.001):
            score += 1
            reasons.append(f"funding favors this side at {funding * 100:.4f}%")
