"""Guard: beaver's marginPct must be a PERCENT in (0,100], never a v2 fraction.

v3.x runtime sizes `(marginPct/100)*withdrawable` (resolve-margin.ts), so a
fraction like 0.25 (the v2 config value!) sizes 0.25% of withdrawable — ~100x
undersize, silent (no 400, since 0.25 still satisfies the (0,100] bound). The v2
producer read marginPct as a FRACTION and multiplied by account_value; the
Runtime 3.0 port must carry the PERCENT (25). marginPct is a pure pass-through
config value here: read at scan.py (inputs.marginPct, default 25) and emitted
