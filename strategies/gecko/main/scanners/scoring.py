...
def build_thesis(coin, candles_15m, candles_1h, candles_4h, funding, sm, inputs):
    ...
    if trend_4h != "NEUTRAL":
        if (direction == "LONG" and trend_4h == "BULLISH") or (direction == "SHORT" and trend_4h == "BEARISH"):
            score += 3
            reasons.append(f"4h trend {trend_4h.lower()} ({trend_strength:.0%} of bars)")
        else:
            score -= 1
    ...
    if trend_1h != "NEUTRAL":
        if (direction == "LONG" and trend_1h == "BULLISH") or (direction == "SHORT" and trend_1h == "BEARISH"):
            score += 2
            reasons.append(f"1h trend {trend_1h.lower()} confirms")
        else:
            score -= 1
    ...
    if direction == "LONG":
        if mom_1h >= 1.0:
            ...
            score += 1; reasons.append(f"1h momentum of {mom_1h:+.2f}%")
        elif mom_1h < -0.5:
    ...
    if sm_dir == direction:
        ...
        score -= 2
        reasons.append(f"smart money opposing, positioned {sm_dir.lower()}")
    ...
    if (direction == "LONG" and funding < 0) or (direction == "SHORT" and funding > 0):
        ...
        score -= 1
        reasons.append("funding shows a crowded trade")
    ...
    if abs(mom_4h) > 1.5:
        if (direction == "LONG" and mom_4h > 0) or (direction == "SHORT" and mom_4h < 0):
            score += 1
            reasons.append(f"4h momentum of {mom_4h:+.1f}%")
