...
def test_both_directions_are_traded(monkeypatch):
    """The cohort is short about as often as it is long. Checked one tick per direction rather
    than one tick emitting both, because at 75% of the wallet a second emit cannot fit — which is
    this package's whole point and is pinned separately below."""
    _gather(monkeypatch, COHORT_OK)
    for asset, direction in (("ETH", "LONG"), ("SOL", "SHORT")):
        _ranked(monkeypatch, [_sig(asset, direction, 95.0)])
        out = scan.scan({}, _ctx())
        assert [(s["asset"], s["direction"]) for s in out] == [(asset, direction)]


def test_conviction_sizes_up_above_the_high_threshold(monkeypatch):
    """93 is the tier, not SIGNALS-HUNTER's 80: above it the score is in the saturated band where
    21 of 116 observations sit at exactly 100, so it is the band worth paying up for."""
    _gather(monkeypatch, COHORT_OK)
    got = {}
    for asset, ts in (("ETH", 92.9), ("SOL", 93.0)):
        _ranked(monkeypatch, [_sig(asset, "LONG", ts)])
        got.update({s["asset"]: s["marginPct"] for s in scan.scan({}, _ctx())})
    assert got == {"ETH": 75.0, "SOL": 90.0}
...
def test_one_qualifying_signal_is_emitted_and_the_rest_are_reported(monkeypatch, capsys):
    """The margin cap enforces the single slot on its own, independently of `slots`: 75 fits under
    92, a second 75 does not. Six qualifying signals must produce ONE open and a line naming the
    five it dropped — silence there would hide a tick that scored well and opened almost nothing."""
    _gather(monkeypatch, COHORT_OK)
    monkeypatch.setattr(scan, "_wallet", lambda ctx: (1000.0, 0.0))   # funded, nothing open yet
    _ranked(monkeypatch, [_sig(f"A{i}", "LONG", 88.0) for i in range(6)])   # base tier, not 93+
    out = scan.scan({}, _ctx())
    assert len(out) == 1 and out[0]["marginPct"] == 75.0
    err = capsys.readouterr().err
    assert "5 qualifying signal(s) not emitted" in err and "Lowest-ranked dropped first" in err


def test_an_open_position_blocks_the_next_tick_without_relying_on_slots(monkeypatch):
    """The counter is seeded with margin ALREADY at risk, so a book holding one 90% position puts
    90 + 90 over the 92 cap. Without that seeding the cap resets every tick and a 15-minute clock
    would be free to stack a second position on top of a full book — the bug this guard was
    rewritten to close, and four times more reachable at this cadence than at an hourly one."""
    _gather(monkeypatch, COHORT_OK)
    monkeypatch.setattr(scan, "_wallet", lambda ctx: (100.0, 900.0))  # 90% already committed
    _ranked(monkeypatch, [_sig("ETH", "LONG", 99.0)])
    assert scan.scan({}, _ctx()) == []


def test_the_one_slot_goes_to_the_best_ranked_not_the_first_seen(monkeypatch):
    """With a single slot this is the whole selection: the emit must be the head of the ranked
    pool. Emitting in arrival order instead would look identical on a healthy tick and quietly
    trade the worst of three."""
    _gather(monkeypatch, COHORT_OK)
    monkeypatch.setattr(scan, "_wallet", lambda ctx: (1000.0, 0.0))   # funded, nothing open yet
    _ranked(monkeypatch, [_sig("BEST", "LONG", 99.0), _sig("MID", "LONG", 92.0),
                          _sig("WORST", "LONG", 86.0)])
    assert [s["asset"] for s in scan.scan({}, _ctx())] == ["BEST"]
...
#
# The payload below is the verbatim shape of a live wallet (a signals-hunter fork, 2026-09-20)
