"""Guard: thesis-fund's marginPct must be a PERCENT in (0,100], never a v2 fraction.

v3.x runtime sizes `(marginPct/100)*withdrawable` (resolve-margin.ts), so a fraction like
v2's 0.12 sizes 0.12% of withdrawable — ~100x undersize, silent (no 400, since 0.12 still
satisfies the (0,100] bound). marginPct is a pure pass-through config value here: read at
scan.py (inputs.marginPct, default 12) and emitted verbatim at the candidate top level;
`scoring.score_thesis` never reads it. So this is a config-level invariant, asserted
directly against the wire-bound value.
"""
...
def test_runtime_margin_pct_is_percent_in_range():
    ...
    # >=1 catches the fraction bug (0.12); <=100 is the wire upper bound.
