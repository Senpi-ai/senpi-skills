"""Guard: grizzly's marginPct must be a PERCENT in (0,100], never a v2 fraction.

v3.x runtime sizes `(marginPct/100)*withdrawable` (resolve-margin.ts), so a
fraction like 0.50 sizes 0.50% of withdrawable — ~100x undersize, silent (no 400,
since 0.50 still satisfies the (0,100] bound). marginPct is a pure pass-through
config value here: read at scan.py (inputs.marginPct, default 50) and emitted
