#!/usr/bin/env python3
# Senpi FOX Scanner v3.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under MIT
"""FOX v3.0 — Contra-Trend Striker.

The Fox lineage: v1.0 was the first profitable agent (+32% on 82 trades).
It died from thesis exit and infrastructure bugs, not bad signals.

Every other Striker in the fleet requires 4H price alignment — if SM says
LONG, the 4H price must already be positive. This filters pump-and-dumps
but also filters REVERSALS.

Fox v3.0 is the opposite: it REQUIRES 4H price to be AGAINST the SM
direction. The thesis: when SM violently enters against the prevailing
trend (rank explosion + contribution explosion WHILE price is going the
other direction), they're front-running a reversal. The price hasn't
caught up yet. This is the highest alpha window.

Why this is different:
- Roach/Jaguar/Mantis: FIRST_JUMP + 4H aligned (momentum following)
- Orca v2.0: FIRST_JUMP + quality confirmation (quality momentum)
- Fox v3.0: FIRST_JUMP + 4H OPPOSED (reversal catching)

Requirements are TIGHTER because contra-trend is riskier:
- Rank jump 20+ (not 15) — must be a massive explosion
- Score 10+ (not 9) — needs extra confirmation
- SM contribution must be growing (contrib_change > 0)
- Trader count 20+ on the asset (broad SM base, not one whale)

Uses: leaderboard_get_markets (single API call per scan)
Runs every 90 seconds.
"""

import json
import sys
import os
import time
from datetime import datetime, timezone

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import fox_config as cfg

TOP_N = 50


# ═══════════════════════════════════════════════════════════════
# HARDCODED CONSTANTS
# ═══════════════════════════════════════════════════════════════

MIN_LEVERAGE = 7
MAX_LEVERAGE = 7
DEFAULT_LEVERAGE = 7
MAX_POSITIONS = 2
MAX_DAILY_ENTRIES = 4
COOLDOWN_MINUTES = 180              # Longer cooldown — contra-trend needs more recovery
MARGIN_PCT = 0.15                   # Smaller margin — higher risk trades
MIN_SCORE = 10                      # Tighter — must have strong confirmation
XYZ_BANNED = True

# Contra-trend Striker thresholds (tighter than normal Striker)
STRIKER_MIN_RANK_JUMP = 20          # 20+ (normal Striker is 15)
STRIKER_MIN_PREV_RANK = 25
STRIKER_MIN_VOL_RATIO = 1.5
MIN_SM_TRADERS = 20                 # Broad SM base required
MIN_CONTRIB_CHANGE = 0.005          # SM contribution must be actively growing


# ═══════════════════════════════════════════════════════════════
# HELPERS
# ═══════════════════════════════════════════════════════════════

def safe_float(val, default=0.0):
    try:
        return float(val)
    except (TypeError, ValueError):
        return default


def now_date():
    return datetime.now(timezone.utc).strftime("%Y-%m-%d")


def now_iso():
    return datetime.now(timezone.utc).isoformat()


def check_contra_trend(direction, price_chg_4h):
    """4H must OPPOSE SM direction — this is the reversal signal."""
    if direction == "LONG" and price_chg_4h < -0.3:
        return True  # SM going LONG while price is falling — reversal
    if direction == "SHORT" and price_chg_4h > 0.3:
        return True  # SM going SHORT while price is rising — reversal
    return False


def time_of_day_modifier():
    hour = datetime.now(timezone.utc).hour
    if 13 <= hour <= 21:
        return 1, "US_SESSION"
    return 0, None


def get_market_in_scan(scan, token, dex):
    for m in scan.get("markets", []):
        if m["token"] == token and m.get("dex", "") == dex:
            return m
    return None


# ═══════════════════════════════════════════════════════════════
# FETCH & PARSE
# ═══════════════════════════════════════════════════════════════

def fetch_markets():
    try:
        data = cfg.mcporter_call("leaderboard_get_markets", limit=100)
        data = data.get("data", data)
        raw = data.get("markets", data)
        if isinstance(raw, dict):
            raw = raw.get("markets", [])
        return raw
    except Exception:
        return None


def parse_scan(raw_markets):
    markets = []
    for i, m in enumerate(raw_markets):
        if not isinstance(m, dict):
            continue
        token = str(m.get("token", m.get("asset", ""))).upper()
        dex = m.get("dex", "")
        if XYZ_BANNED and (dex == "xyz" or token.lower().startswith("xyz:")):
            continue
        if not token:
            continue

        markets.append({
            "token": token,
            "dex": dex,
            "rank": i + 1,
            "direction": str(m.get("direction", "")).upper(),
            "contribution": safe_float(m.get("pct_of_top_traders_gain", 0)),
            "traders": int(m.get("trader_count", 0)),
            "price_chg_4h": safe_float(m.get("token_price_change_pct_4h", 0)),
            "price_chg_1h": safe_float(m.get("token_price_change_pct_1h",
                                       m.get("price_change_1h", 0))),
            "contrib_change": safe_float(m.get("contribution_pct_change_4h", 0)),
        })

    return {"markets": markets[:TOP_N], "time": now_iso()}


def check_asset_volume(token, dex):
    try:
        data = cfg.mcporter_call("market_get_asset_data",
                                  asset=token, candle_intervals=["1h"],
                                  include_funding=False)
        if not data:
            return 0, True
        ad = data.get("data", data)
        if not isinstance(ad, dict):
            return 0, True
        ac = ad.get("asset_context", ad.get("assetContext", {}))
        if not isinstance(ac, dict):
            return 0, True
        vol = safe_float(ac.get("dayNtlVlm", 0))
        prev = safe_float(ac.get("prevDayNtlVlm", 0))
        if prev > 0:
            ratio = vol / prev
            return ratio, ratio >= STRIKER_MIN_VOL_RATIO
        return 0, True
    except Exception:
        return 0, True


# ═══════════════════════════════════════════════════════════════
# CONTRA-TREND STRIKER DETECTION
# ═══════════════════════════════════════════════════════════════

def detect_contra_signals(current_scan, history):
    """Detect FIRST_JUMP where SM is OPPOSING the 4H price trend."""

    prev_scans = history.get("scans", [])
    if not prev_scans:
        return []

    latest_prev = prev_scans[-1]
    oldest_available = prev_scans[-min(len(prev_scans), 5)]

    prev_top50_tokens = set()
    for m in latest_prev.get("markets", []):
        prev_top50_tokens.add((m["token"], m.get("dex", "")))

    signals = []

    for market in current_scan.get("markets", []):
        token = market["token"]
        dex = market.get("dex", "")
        current_rank = market["rank"]
        direction = market["direction"]
        current_contrib = market["contribution"]
        traders = market["traders"]
        contrib_change = market.get("contrib_change", 0)

        if current_rank <= 10:
            continue

        # THE KEY DIFFERENCE: 4H must OPPOSE SM direction
        if not check_contra_trend(direction, market.get("price_chg_4h", 0)):
            continue

        # SM must be broad — not a single whale
        if traders < MIN_SM_TRADERS:
            continue

        # SM contribution must be growing — they're actively entering
        if contrib_change < MIN_CONTRIB_CHANGE:
            continue

        prev_market = get_market_in_scan(latest_prev, token, dex)
        old_market = get_market_in_scan(oldest_available, token, dex)
        if not prev_market:
            continue

        rank_jump = prev_market["rank"] - current_rank

        # Tighter rank jump gate for contra-trend
        if rank_jump < STRIKER_MIN_RANK_JUMP:
            continue

        is_first_jump = False
        is_immediate = False
        is_contrib_explosion = False
        reasons = []

        if prev_market["rank"] >= STRIKER_MIN_PREV_RANK:
            is_immediate = True
            reasons.append(f"CONTRA_MOVER +{rank_jump} from #{prev_market['rank']}")

            was_in_prev = (token, dex) in prev_top50_tokens
            if not was_in_prev or prev_market["rank"] >= 30:
                is_first_jump = True
                reasons.append(f"FIRST_JUMP #{prev_market['rank']}->#{current_rank}")

        if prev_market["contribution"] > 0:
            contrib_ratio = current_contrib / prev_market["contribution"]
            if contrib_ratio >= 3.0:
                is_contrib_explosion = True
                reasons.append(f"CONTRIB_EXPLOSION {contrib_ratio:.1f}x")

        if not is_first_jump and not is_immediate:
            continue

        # Contribution velocity
        contrib_velocity = 0
        recent_contribs = []
        for scan in prev_scans[-5:]:
            m = get_market_in_scan(scan, token, dex)
            if m:
                recent_contribs.append(m["contribution"])
        recent_contribs.append(current_contrib)
        if len(recent_contribs) >= 2:
            deltas = [recent_contribs[i + 1] - recent_contribs[i]
                      for i in range(len(recent_contribs) - 1)]
            contrib_velocity = sum(deltas) / len(deltas) * 100

        # ── Scoring ──
        score = 0

        # Contra-trend premium — the price is AGAINST you, so SM must be overwhelming
        if is_first_jump:
            score += 3
        if is_immediate:
            score += 2
        if is_contrib_explosion:
            score += 2
        if abs(contrib_velocity) > 10:
            score += 2
            reasons.append(f"HIGH_VELOCITY {abs(contrib_velocity):.1f}")

        # Deep origin bonus
        if prev_market["rank"] >= 40:
            score += 1
            reasons.append("DEEP_CLIMBER")

        # Multi-scan climb
        if old_market:
            total_climb = old_market["rank"] - current_rank
            if total_climb >= 10:
                score += 1
                reasons.append(f"CLIMBING +{total_climb} over scans")

        # Contra-trend context — how strong is the opposing 4H?
        p4h = abs(market.get("price_chg_4h", 0))
        if p4h >= 2.0:
            score += 1
            reasons.append(f"STRONG_COUNTER {market['price_chg_4h']:+.1f}% 4H vs SM {direction}")

        # SM depth
        if traders >= 50:
            score += 1
            reasons.append(f"DEEP_SM ({traders}t)")

        # Contribution actively growing
        if contrib_change >= 0.02:
            score += 1
            reasons.append(f"CONTRIB_GROWING +{contrib_change*100:.1f}%")

        # Time of day
        tod_mod, tod_reason = time_of_day_modifier()
        score += tod_mod
        if tod_reason:
            reasons.append(tod_reason)

        if score < MIN_SCORE or len(reasons) < 4:
            continue

        # Volume confirmation
        vol_ratio, vol_strong = check_asset_volume(token, dex)
        if not vol_strong:
            continue
        reasons.append(f"VOL_CONFIRMED {vol_ratio:.1f}x")

        signals.append({
            "token": token,
            "dex": dex if dex else None,
            "direction": direction,
            "mode": "CONTRA_STRIKER",
            "score": score,
            "reasons": reasons,
            "currentRank": current_rank,
            "rankJump": rank_jump,
            "isFirstJump": is_first_jump,
            "isContribExplosion": is_contrib_explosion,
            "contribVelocity": round(contrib_velocity, 4),
            "volRatio": round(vol_ratio, 2),
            "contribution": round(current_contrib * 100, 3),
            "traders": traders,
            "priceChg4h": market.get("price_chg_4h", 0),
            "contribChange4h": contrib_change,
        })

    signals.sort(key=lambda s: s["score"], reverse=True)
    return signals


# ═══════════════════════════════════════════════════════════════
# TRADE COUNTER
# ═══════════════════════════════════════════════════════════════

def load_trade_counter():
    p = os.path.join(cfg.STATE_DIR, "trade-counter.json")
    if os.path.exists(p):
        try:
            with open(p) as f:
                return json.load(f)
        except (json.JSONDecodeError, IOError):
            pass
    return {"date": now_date(), "entries": 0}


def save_trade_counter(tc):
    if tc.get("date") != now_date():
        tc = {"date": now_date(), "entries": 0}
    cfg.atomic_write(os.path.join(cfg.STATE_DIR, "trade-counter.json"), tc)


# ═══════════════════════════════════════════════════════════════
# MAIN
# ═══════════════════════════════════════════════════════════════

def run():
    wallet, strategy_id = cfg.get_wallet_and_strategy()
    if not wallet:
        cfg.output({"status": "ok", "heartbeat": "NO_REPLY", "note": "no wallet"})
        return

    account_value, positions = cfg.get_positions(wallet)
    if account_value <= 0:
        cfg.output({"status": "ok", "heartbeat": "NO_REPLY", "note": "cannot read account"})
        return

    if len(positions) >= MAX_POSITIONS:
        coins = [p["coin"] for p in positions]
        cfg.output({"status": "ok", "heartbeat": "NO_REPLY",
                     "note": f"RIDING: {coins}. DSL manages exit.",
                     "_v2_no_thesis_exit": True})
        return

    tc = load_trade_counter()
    if tc.get("date") != now_date():
        tc = {"date": now_date(), "entries": 0}
    if tc.get("entries", 0) >= MAX_DAILY_ENTRIES:
        cfg.output({"status": "ok", "heartbeat": "NO_REPLY",
                    "note": f"Daily entry limit ({MAX_DAILY_ENTRIES}) reached"})
        return

    raw_markets = fetch_markets()
    if raw_markets is None:
        cfg.output({"status": "error", "error": "failed to fetch markets"})
        return

    current_scan = parse_scan(raw_markets)
    history = cfg.load_scan_history()

    signals = detect_contra_signals(current_scan, history)

    history["scans"].append(current_scan)
    cfg.save_scan_history(history)

    signals = [s for s in signals
               if not cfg.is_asset_cooled_down(s["token"], COOLDOWN_MINUTES)]
    held_coins = {p["coin"].upper() for p in positions}
    signals = [s for s in signals if s["token"] not in held_coins]

    if not signals:
        cfg.output({"status": "ok", "heartbeat": "NO_REPLY",
                    "note": f"No contra-trend signals. Scanned {len(current_scan['markets'])} markets.",
                    "scansInHistory": len(history["scans"])})
        return

    best = signals[0]
    margin = round(account_value * MARGIN_PCT, 2)

    tc["entries"] = tc.get("entries", 0) + 1
    save_trade_counter(tc)

    cfg.output({
        "status": "ok",
        "signal": best,
        "entry": {
            "asset": best["token"],
            "direction": best["direction"],
            "leverage": DEFAULT_LEVERAGE,
            "margin": margin,
            "orderType": "FEE_OPTIMIZED_LIMIT",
        },
        "constraints": {
            "maxPositions": MAX_POSITIONS,
            "maxLeverage": MAX_LEVERAGE,
            "maxDailyEntries": MAX_DAILY_ENTRIES,
            "cooldownMinutes": COOLDOWN_MINUTES,
            "xyzBanned": XYZ_BANNED,
            "_v2_no_thesis_exit": True,
            "_note": "DSL managed by plugin runtime. Scanner does NOT manage exits.",
        },
        "_fox_version": "3.0",
    })


if __name__ == "__main__":
    try:
        run()
    except Exception as e:
        cfg.log(f"CRITICAL ERROR: {e}")
        import traceback
        traceback.print_exc(file=sys.stderr)
        cfg.output({"status": "error", "error": str(e)})
