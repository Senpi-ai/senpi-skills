...
| `{MID_MODEL}` | Mid-tier model ID | `anthropic/claude-sonnet-4-20250514` |
| `{BUDGET_MODEL}` | Budget-tier model ID | `anthropic/claude-sonnet-4-20250514` |

## 1. Emerging Movers — Primary / Main Session
...
```json
{
  ...
  "payload": {
    "kind": "systemEvent",
    "text": "FOX scanner: Run `PYTHONUNBUFFERED=1 python3 {SCRIPTS}/fox-emerging-movers.py`, parse JSON.\n\nSLOT GUARD (MANDATORY): Check `anySlotsAvailable` BEFORE any entry. If false → HEARTBEAT_OK.\n\nv0.3 ENTRY FILTERS (apply to ALL signals before scoring):\n1. RANK JUMP MINIMUM: `rankJumpThisScan ≥ 15` OR `contribVelocity > 15`. If neither → skip signal.\n1b. FJ PERSISTENCE FILTER (v0.3): Before acting on any FIRST_JUMP, check `{WORKSPACE}/fj-last-seen.json`. Only enter if the token appeared in the PREVIOUS scan too. New FJs are logged to fj-last-seen.json but NOT acted on — must persist across two consecutive 3min scans. After processing, save current scan FJ tokens to fj-last-seen.json.\n2. TIME-OF-DAY SCORING: add +1 pt if current hour (UTC) is 04:00-13:59. Subtract 2 pts if 18:00-01:59.\n3. Standard scoring: FIRST_JUMP +3, IMMEDIATE +2, vel>10 +2, CONTRIB_EXPLOSION +2, DEEP_CLIMBER +1, other reasons +1 each, BTC alignment +1.\n4. Score threshold: ≥6 pts (≥8 for NEUTRAL regime).\n5. Max leverage ≥ 7x (check max-leverage.json). Hourly trend alignment.\n\nRead `{WORKSPACE}/fox-trade-counter.json` for percentage-based margin tier (marginPct × strategy budget). Route to best-fit strategy from `{WORKSPACE}/fox-strategies.json`.\nOn entry: create DSL state with conviction-scaled Phase 1 (v7.2). Store `score` in DSL state. Phase 1 tolerance auto-scales: score 6-7 → 0.03/lev/30min, score 8-9 → 0.035/lev/45min, score 10+ → 0.04/lev/60min.\n\nRE-ENTRY CHECK (v7.2): If a recently-exited Phase 1 trade's asset is still in scanner output with same direction AND contribVelocity > 5 AND price moved further in original direction AND original loss ≤ 15% ROE AND within 2h of exit → re-enter at 75% margin, score threshold 5 pts (relaxed). Set isReentry=true in DSL state. isFirstJump NOT required for re-entries.\nAlert {TELEGRAM}. If no actionable signals → HEARTBEAT_OK."
  }
}
```
...
## 6. Opportunity Scanner — Primary / Main Session
...
|---|------|----------|---------|-------------|-------------|-------|
| 1 | Emerging Movers | */3 * * * * | main | systemEvent | `text` | Primary |
...
| 6 | Opp Scanner | */15 * * * * | main | systemEvent | `text` | Primary |
| 7 | Market Regime | 0 * * * * | isolated | agentTurn | `message` | Mid |
