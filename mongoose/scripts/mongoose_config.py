"""MONGOOSE v1.0 — Shared config + MCP shim + helpers wrapper (LEG-AWARE).

MONGOOSE — On-Chain Finance vs Legacy Hedge Fund. ONE producer script driven by
the MONGOOSE_LEG env var into one of two books, each bound to its own wallet +
runtime + DSL:

  MONGOOSE_LEG=long  -> "On-chain" book. LONGS the on-chain financial rails on
                    Hyperliquid: HYPE (the venue), CRCL (Circle/stablecoins),
                    COIN (Coinbase), HOOD (Robinhood), MSTR + PURRDAT (crypto
                    treasuries), trend-confirmed.
                    config/mongoose-long-config.json -> mongoose_long_signals
  MONGOOSE_LEG=short -> "Legacy" book. SHORTS legacy finance + broad
                    financial-beta — BX (Blackstone) + SP500 — trend-confirmed
                    (capitulation guard).
                    config/mongoose-short-config.json -> mongoose_short_signals

The thesis: money is migrating ON-CHAIN — stablecoins, crypto exchanges, BTC
treasuries — eating legacy financial rails. LONG the disruptors, SHORT the
incumbents + broad index — the P&L is the disruptor-vs-incumbent spread. The
hedge is CROSS-SECTOR with a thin short universe (it leans on the index). The
long/short balance is an explicit operator decision (see config `_hedge_note`),
defaulting to a net-long tilt. NOT a copy-trader — each book scores its own universe.
...
race-dedup cache. The producer (mongoose-producer.py) owns the thematic universe
...
# ─── Leg resolution ──────────────────────────────────────────
LEG = (os.environ.get("MONGOOSE_LEG") or "long").strip().lower()
if LEG not in ("long", "short"):
    raise RuntimeError(
        f"MONGOOSE_LEG must be 'long' or 'short' (got {LEG!r}). "
        "Set it on the runtime host before starting the producer daemon."
    )

WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE", "/data/workspace")
SKILL_DIR = Path(WORKSPACE) / "skills" / "mongoose-strategy"
CONFIG_PATH = SKILL_DIR / "config" / f"mongoose-{LEG}-config.json"
STATE_DIR = SKILL_DIR / "state"
RECENT_SIGNALS_PATH = STATE_DIR / f"recent-signals-{LEG}.json"

STATE_DIR.mkdir(parents=True, exist_ok=True)

_WALLET_ENV = "MONGOOSE_LONG_WALLET" if LEG == "long" else "MONGOOSE_SHORT_WALLET"
_STRATEGY_ENV = "MONGOOSE_LONG_STRATEGY_ID" if LEG == "long" else "MONGOOSE_SHORT_STRATEGY_ID"
...
def _get_wrapper_client() -> SenpiClient:
    if not os.environ.get("SENPI_AUTH_TOKEN", "").strip():
        raise RuntimeError(
            "SENPI_AUTH_TOKEN is not set. Mongoose's MCP calls and signal POST "
            "both require it. Set it on the runtime host before starting the "
            "producer daemon."
        )
    client = SenpiClient()
    log_event("mongoose_wrapper_enabled", leg=LEG, sdk_path=_sdk_path)
    return client
...
def mcp_call(tool, **params):
    try:
        ...
        sys.stderr.write(
            f"[mongoose-v1:{LEG}] mcp_call_failed tool={tool} "
...
def get_positions(wallet):
    ...
    Mongoose holds both xyz equities and main-DEX crypto on the same wallet, so
...
def log(msg):
    print(f"[mongoose-v1:{LEG}] {msg}", file=sys.stderr, flush=True)
