#!/usr/bin/env python3
# Senpi MONGOOSE Producer v1.0.0
# Copyright 2026 Senpi (https://senpi.ai)
# Licensed under Apache-2.0
# Source: https://github.com/Senpi-ai/senpi-skills
"""MONGOOSE v1.0.0 Producer — On-Chain Finance vs Legacy (long/short).

Mongoose bets that money is migrating ON-CHAIN: crypto exchanges, stablecoin
issuers, and BTC treasuries are eating legacy financial rails. ONE producer
script serves both books; the MONGOOSE_LEG env var selects which:

  MONGOOSE_LEG=long   "On-chain" — LONG the on-chain financial rails: HYPE (the
                  venue), CRCL (Circle/stablecoins), COIN (Coinbase), HOOD
                  (Robinhood), MSTR + PURRDAT (crypto treasuries), trend-confirmed.
  MONGOOSE_LEG=short  "Legacy" — SHORT legacy finance + broad financial-beta:
                  BX (Blackstone) + SP500, trend-confirmed (capitulation guard).

Each book scores its OWN curated universe on ABSOLUTE trend confirmation, with
cross-sectional relative strength as a tiebreaker. Per-group SIZING WEIGHTS
express conviction (HYPE + CRCL big; MSTR/PURRDAT — levered BTC/HYPE proxies —
sized down) without hardcoding dollar amounts — every size scales with account
value. The edge is the disruptor-vs-incumbent spread.

NOTE — the hedge is CROSS-SECTOR and the short universe is THIN (few pure
legacy-finance names list on the venue), so the short leg leans on the broad
index; it nets down market beta but is a looser hedge than a same-sector pair.

The long/short balance is an explicit operator decision — set by the wallet
funding split (see config `_hedge_note`); the default is a net-long tilt.
...
Environment / config resolution:
  MONGOOSE_LEG            — REQUIRED. "long" or "short".
  SENPI_AUTH_TOKEN    — REQUIRED. Bearer token for MCP + signal POST.
  MONGOOSE_LONG_WALLET    — long-book strategy wallet (or config.wallet)
  MONGOOSE_SHORT_WALLET   — short-book strategy wallet (or config.wallet)
  MONGOOSE_DECISION_MODEL — bare LLM model name; resolved into runtime.yaml
...
SCANNER_NAME = f"mongoose_{LEG}_signals"
SIGNAL_TYPE = "MONGOOSE_ONCHAIN_LONG" if LEG == "long" else "MONGOOSE_LEGACY_SHORT"
DIRECTION = "LONG" if LEG == "long" else "SHORT"

NORM_DIV = 9.0

# ── Curated thematic universes (config.universe overrides) — on-chain finance ──
# LONG = the on-chain financial rails (crypto exchanges, stablecoin issuers, BTC
#        treasuries, HYPE) — the disruptors eating legacy finance.
# SHORT = legacy finance + broad financial-beta (the incumbents being disrupted).
# A cross-sector hedge: the short universe is thin (few pure legacy-finance names
# on the board), so it leans on the broad index. Names not live are skipped.
_HAVES = [
    "HYPE",          # the on-chain venue itself (main DEX)
    "xyz:CRCL",      # Circle — USDC / stablecoin rails
    "xyz:COIN",      # Coinbase — the crypto exchange
    "xyz:HOOD",      # Robinhood — crypto-native retail brokerage
    "xyz:MSTR",      # Strategy — corporate BTC treasury
    "xyz:PURRDAT",   # Hyperliquid Strategies — HYPE treasury
]
_HAVE_NOTS = [
    "xyz:BX",        # Blackstone — legacy alternative-asset finance
    "xyz:SP500",     # broad financial-beta (the incumbent market)
]
...
_DEFAULTS = {
    "long": {
        ...
        "rsThresholdPct": 3.0,
        "rsiOverbought": 84,           # crypto-finance equities run hot; loosen blow-off
    },
    "short": {
        "universe": _HAVE_NOTS,
...
# ═══════════════════════════════════════════════════════════════
#
# Unlike a pure cross-sectional book (Cougar), Mongoose's universe is THEMATIC: every
...
if __name__ == "__main__":
    ...
    _kwargs = {
        ...
        "name": f"mongoose-{LEG}-producer-{_lock_id}",
